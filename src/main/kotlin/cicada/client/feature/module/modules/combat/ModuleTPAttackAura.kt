...
object ModuleTPAttackAura : ClientModule("TPAttackAura", ModuleCategory.COMBAT) {
    val targetFinder = tree(TargetFinder()).apply {
        searchRange = float("SearchRange", 100f, 0f..200f)
    }
    private val attackRange by float("AttackRange", 6f, 0f..6f)
    private val tpStep by float("TPStep", 3f, 0f..20f)
        ...
        if (event is EventTick.Pre) {
            ...
            if (targetFinder.target != null && targetFinder.target!!.hurtTime <= 0 && player.getAttackStrengthScale(0.5f) > 0.9) {
                ...
                path = AStar.findPath(
                    ...
                    { x, y, z ->
                        !level.getBlockState(BlockPos(x, y, z)).block.hasCollision
                    }
                )?.map { Vec3(it.x + 0.5, it.y + 0.1, it.z + 0.5) }
