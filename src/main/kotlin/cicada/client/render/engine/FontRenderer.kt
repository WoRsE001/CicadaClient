...
private fun FontData.fallbackAdvance(charCode: Int): Float =
    if (charCode == ' '.code) metrics.lineHeight * 0.28f else 0f
...
private fun FontData.addVertices(vertexConsumer: VertexConsumer, pose: Matrix3x2fc, text: String, x: Float, y: Float, size: Float, color: Int) {
    ...
    for (i in text.indices) {
        ...
        val glyph = glyphs[charCode]

        if (glyph == null) {
            xOffset += fallbackAdvance(charCode) * size
            continue
        }

        if (glyph.planeLeft != glyph.planeRight && glyph.planeTop != glyph.planeBottom) {
...
fun GuiGraphicsExtractor.text(pose: Matrix3x2fc, fontData: FontData?, text: String, x: Float, y: Float, size: Float, color: Int) {
    ...
    guiRenderState.addGuiElement(
        TexturedElementToDraw(
            pose,
            MSDF_IMAGE_PIPELINE,
            x, y, fontData.visualWidth(text, size), fontData.height(size),
            { vertexConsumer ->
...
fun GuiGraphicsExtractor.centeredText(pose: Matrix3x2fc, fontData: FontData?, text: String, x: Float, y: Float, size: Float, color: Int) {
    ...
    text(pose, fontData, text, x - fontData.visualWidth(text, size) / 2, y - fontData.height(size) / 2, size, color)
}
...
fun FontData.width(text: String, size: Float): Float {
    ...
    for (i in text.indices) {
        ...
        if (i > 0) {
            val prevCode = text[i - 1].code
            w += (kernings[kerningKey(prevCode, charCode)] ?: 0f) * size
        }

        val glyph = glyphs[charCode]
        w += (glyph?.advance ?: fallbackAdvance(charCode)) * size
    }
    return w
}

fun FontData.visualWidth(text: String, size: Float): Float {
    var xOffset = 0f
    var minX: Float? = null
    var maxX = 0f
    ...
    for (i in text.indices) {
        ...
        if (i > 0) {
            val prevCode = text[i - 1].code
            xOffset += (kernings[kerningKey(prevCode, charCode)] ?: 0f) * size
        }
        ...
        if (glyph.planeLeft != glyph.planeRight) {
            val x0 = xOffset + glyph.planeLeft * size
            val x1 = xOffset + glyph.planeRight * size
